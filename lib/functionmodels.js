...
function setCallModelFree(concreteFn, symbolicFn, concretizer = getConcrete) {
    if (!symbolicFn) {
        callModels.set(concreteFn, null);
        return;
    }
    ...
    const shim = function(...args) {
        ...
        return new Concolic(concreteFn.apply(cbase, cargs), new symbolicFn(...sargs));
    };
    callModels.set(concreteFn, shim);
}
...
setCallModel(String.prototype.toLowerCase, StringToLowerCase)
setCallModel(String.prototype.toUpperCase, StringToUpperCase)
setCallModel(String.prototype.charCodeAt, StringCharCodeAt);

// FIXME: this is just a temporary fix for String.prototype.match().
callModels.set(String.prototype.match, function(...args) {
    const cbase = getConcrete(this),
        sbase = getSymbolic(this);
    const cargs = _.map(args, getConcrete),
        sargs = _.map(args, getSymbolic);

    return new Concolic(String.prototype.match.apply(cbase, cargs), new RegExpExec(sargs[0], sbase));
});
...
callModels.set(String.prototype.replace, function(searchValue, replaceValue) {
    ...
    const cresult = String.prototype.replace.call(cbase, csearchValue, creplaceValue);
    // huzi add, NOTE: we do not handle the situation _.isFunction(creplaceValue)
    // if (_.isRegExp(csearchValue) || _.isFunction(creplaceValue)) {
    //     // NOTE: this is, strictly speaking, incorrect.
    //     return String(cbase) === cresult ? this : cresult;
    // } else {
...
setCallModel(RegExp.prototype.exec, RegExpExec);
...
setCallModel(Number.prototype.toFixed, NumberToFixed);
...
setCallModel(Array.prototype.includes, ArrayIncludes);
...
setCallModelFree(Math.abs, MathAbs);
setCallModelFree(Math.floor, MathFloor);
setCallModelFree(Math.ceil, MathCeil);
setCallModelFree(Math.round, MathRound);
setCallModelFree(Math.sqrt, MathSqrt);

setCallModel(Number.prototype.toFixed, NumberToFixed);

setCallModelFree(isFinite, IsFinite);
setCallModelFree(Number.isFinite, NumberIsFinite);
setCallModelFree(isNaN, IsNaN);
setCallModelFree(Number.isNaN, NumberIsNaN);
