...
// huzi add, when \b appeared in regex (eg. /.\b./), the dot may not
// match \0. 
// FIXME: this implementation is also unstable
...
class WordBoundary extends Assertion {
    // huzi add, FIXME : sound but not complete
...
class NegatedWordBoundary extends WordBoundary {
    // huzi add, FIXME : sound but not complete
    // toRegexFormula() { return WORD_CLASS }
}
...
class CharRange {
    ...
    toRegexFormula(negate) {
        ...
        if (!negate) {
            ...
            const startAscii = (start.charCodeAt() - 1 < 0) ? 0 : (start.charCodeAt() - 1);
            const endAscii = (end.charCodeAt() + 1 > 255) ? 0 : (end.charCodeAt() + 1);
            const negStart = String.fromCharCode(startAscii)
            const negEnd = String.fromCharCode(endAscii)
            return union([range("\0", negStart), range(negEnd, "\xFF")]);
        }
